...
//////////////////////////////////////////////
	...
	public static class UpdaterHandler {
		...
		public static volatile boolean updateFound = false;
		public static volatile String updateVersion = "unknown";
		public static volatile boolean updateAlreadyDownloaded = false;
		
		private static volatile Plugin plugin;
		private static volatile int projectId;
		private static volatile String pluginChatPrefix;
		private static volatile File pluginFile;
		private static volatile ChatColor primaryColor;
		private static volatile String updateCommand; // e.g. /plugincommand update
		private static volatile String bukkitDevSlug; // the String after .../bukkit-plugins/{this one}
		...
		public static void notifyIfUpdateWasFound(final Player player, String updatePermission) {
			...
			if (updateFound && !updateAlreadyDownloaded && player.hasPermission(updatePermission)) {				
				Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() { public void run() {
					player.sendMessage(pluginChatPrefix + primaryColor + "Found an update: v" + updateVersion + "   §7(Yours: v" + plugin.getDescription().getVersion() + ")");
		...
		public static void manuallyCheckUpdates(final CommandSender sender) {
